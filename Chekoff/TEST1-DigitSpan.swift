import SwiftUI

struct DigitSpanTestView: View {
    enum Phase {
        case showing    // "Get ready…" and digit presentation
        case inputting  // user entering
        case feedback   // waiting for retry or auto-advance
    }
    
    @State private var sequence: [Int] = []
    @State private var currentIndex: Int = -1
    @State private var userInput: [Int] = []
    @State private var phase: Phase = .showing
    @State private var feedback: String? = nil
    @State private var currentLength = 4
    @State private var maxScore = 0
    
    @State private var isDigitVisible: Bool = false
    @State private var showGetReady = true
    
    // Track the running async task so we can cancel it
    @State private var displayTask: Task<Void, Never>? = nil

    var body: some View {
        VStack {
            Text("Digit Span Test")
                .font(.largeTitle)
                .padding(.top, 20)
            
            Spacer()
            
            // Main content area
            Group {
                switch phase {
                case .showing:
                    if currentIndex >= 0 && currentIndex < sequence.count && isDigitVisible {
                        Text("\(sequence[currentIndex])")
                            .id(currentIndex) // force SwiftUI to treat each digit as a new view
                            .font(.system(size: 60, weight: .bold))
                            .transition(.opacity)
                    } else if showGetReady {
                        Text("Get ready…")
                            .font(.title2)
                            .foregroundColor(.secondary)
                    }
                    
                case .inputting:
                    VStack {
                        Text("Enter the digits:")
                        
                        // Show entered digits
                        HStack {
                            ForEach(userInput.indices, id: \.self) { i in
                                Text("\(userInput[i])")
                                    .font(.title2)
                                    .padding(5)
                            }
                        }
                        .frame(height: 40)
                        
                        Spacer()
                        
                        // Keypad
                        LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 4), count: 3), spacing: 4) {
                            ForEach(1..<10) { number in
                                Button { userInput.append(number) } label: {
                                    Text("\(number)")
                                        .font(.title)
                                        .frame(width: 80, height: 80)
                                        .background(Color.gray.opacity(0.2))
                                        .cornerRadius(12)
                                }
                            }
                            
                            Button("Clear") {
                                userInput.removeAll()
                            }
                            .font(.title3)
                            .frame(width: 80, height: 80)
                            .background(Color.gray.opacity(0.2))
                            .cornerRadius(12)
                            
                            Button { userInput.append(0) } label: {
                                Text("0")
                                    .font(.title)
                                    .frame(width: 80, height: 80)
                                    .background(Color.gray.opacity(0.2))
                                    .cornerRadius(12)
                            }
                            
                            Button("OK") {
                                checkAnswer()
                            }
                            .font(.title3)
                            .frame(width: 80, height: 80)
                            .background(Color.gray.opacity(0.2))
                            .cornerRadius(12)
                        }
                    }
                    
                case .feedback:
                    if let feedback = feedback {
                        if feedback == "Correct!" {
                            Text(feedback)
                                .font(.title3)
                                .foregroundColor(.green)
                        } else {
                            Button(action: { startNewRound() }) {
                                Text(feedback)
                                    .font(.title3)
                                    .foregroundColor(.red)
                            }
                            .buttonStyle(.plain)
                        }
                    }
                }
            }
            .frame(maxHeight: .infinity)
            
            Spacer()
            
            // Sequence length controls (always pinned here)
            HStack {
                Button {
                    let newLen = max(2, currentLength - 1)
                    startNewRound(setLength: newLen)
                } label: {
                    Image(systemName: "minus.circle.fill")
                        .font(.title2)
                        .foregroundColor(.red)
                }
                
                Text("Sequence Length: \(currentLength)")
                    .font(.headline)
                    .padding(.horizontal)
                
                Button {
                    let newLen = currentLength + 1
                    startNewRound(setLength: newLen)
                } label: {
                    Image(systemName: "plus.circle.fill")
                        .font(.title2)
                        .foregroundColor(.green)
                }
            }
            .padding(.bottom, 10)
            
            Text("Max Score: \(maxScore)")
                .padding(.bottom, 20)
        }
        .padding()
        .onAppear {
            startNewRound()
        }
        .onDisappear {
            // Cancel any running display task when leaving screen
            displayTask?.cancel()
        }
    }
    
    // MARK: - Logic
    
    private func startNewRound(setLength: Int? = nil) {
        // Cancel any currently running digit-display task
        displayTask?.cancel()
        
        if let setLength { currentLength = setLength }
        let len = currentLength
        
        sequence = (0..<len).map { _ in Int.random(in: 0...9) }
        userInput.removeAll()
        feedback = nil
        currentIndex = -1
        isDigitVisible = false
        phase = .showing
        showGetReady = true
        
        let showTime: Double = 0.8
        let hideTime: Double = 0.4   // slightly longer hide for clarity

        displayTask = Task { @MainActor in
            do {
                // Initial delay for "Get ready..." text
                try await Task.sleep(for: .seconds(1.0))
                try Task.checkCancellation()
                
                withAnimation { self.showGetReady = false }
                
                // Show each digit in sequence
                for i in 0..<sequence.count {
                    try Task.checkCancellation()
                    
                    // short pause before showing
                    try await Task.sleep(for: .seconds(hideTime))
                    
                    self.currentIndex = i   // set the digit first
                    withAnimation {
                        self.isDigitVisible = true
                    
                    }
                    
                    // visible period
                    try await Task.sleep(for: .seconds(showTime))
                    
                    withAnimation {
                        self.isDigitVisible = false
                    }
                }
                
                // small pause after last digit
                try await Task.sleep(for: .seconds(hideTime))
                try Task.checkCancellation()
                
                withAnimation {
                    self.phase = .inputting
                }
            } catch {
                // Task was cancelled, just exit
            }
        }
    }
    
    private func checkAnswer() {
        if userInput.count < sequence.count {
            feedback = "Not enough digits"
            phase = .feedback
            return
        }
        
        if userInput == sequence {
            feedback = "Correct!"
            maxScore = max(maxScore, currentLength)
            phase = .feedback
            
            // Start next round after short delay
            displayTask?.cancel()
            displayTask = Task { @MainActor in
                try? await Task.sleep(for: .seconds(1.5))
                startNewRound()
            }
        } else {
            feedback = "Try again"
            phase = .feedback
        }
    }
}
